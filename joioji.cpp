...
namespace Solution_6065476225470371 {
    auto solve() -> void {
        ...
        for (i32 len = N; len > 0; len--) {
            for (i32 i = 0; i < N; i++) {
                auto j = i + len - 1;
                if (j >= N) break;
                auto cntj = cnt_ps[0][j] - (i == 0? 0: cnt_ps[0][i - 1]);
                auto cnto = cnt_ps[1][j] - (i == 0? 0: cnt_ps[1][i - 1]);
                auto cnti = cnt_ps[2][j] - (i == 0? 0: cnt_ps[2][i - 1]);
                if (cntj == cnto and cnto == cnti) {
                    std::cout << len << endl;
                    return;
                }
            }
