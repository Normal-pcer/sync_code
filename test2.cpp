...
class SegTree {
    ...
    int mid(int p) const { return std::midpoint(tr[p].begin, tr[p].end); }
#define ls (p<<1)
#define rs (p<<1 | 1)
    void pushUp(int p) {
    ...
    void build(int begin, int end, int p = 1) {
        ...
        build(begin, mid(p), ls), build(mid(p), end, rs);
        pushUp(p);
    }
public:
    SegTree(int N, std::vector<long long> &init): tr(N << 2), init(init) {
        build(0, N);
    }
