...
namespace Solution {
    ...
    // ll F[_N][_K][_K];  // 到达第 i 个站点，此前的两次选择是 j 和 k，最小的开支

    // 从 p 到 n 的最小开支
    ll mem[_N][_K][_K];
    ll dfs(int p, int prev, int prev2) { 
        if (p == N-1)  return 0LL;  // 抵达终点
        if (mem[p][prev][prev2] != -1)  return mem[p][prev][prev2];
        ll ans = infLL;
        auto next = p+1;
    ...
    void solve() {
        ...
        io << dfs(0, K, K) << endl;
        // std::memset(F, 0x3f, sizeof(F)), F[0][K][K] = 0;
        // from(i, 1, N-1) {
        //     for (auto &cur: graph[i-1].ways) {
        //         from(prev, 0, K) {
        //             from(prev2, 0, K) {
        //                 ll co = graph[i-1].dis * cost[cur];
        //                 if (cur == prev) {}
        //                 else if (cur == prev2 and graph[i-1].dis * rv[prev] <= T) {}
        //                 else {
        //                     co += w[cur];
        //                 }
        //                 chkMax(F[i][cur][prev], F[i-1][prev][prev2] + co);
        //             }
        //         }
        //     }
        // }
