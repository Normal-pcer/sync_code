...
int F[1<<_N][1<<_N];    // 选手 1 获得的最高积分；其中，i 表示是否与对应选手比赛过，j 表示是否胜利
...
int main(int argc, char const *argv[]) {
    ...
    while (T --> 0) {
        ...
        from(i, 0, (1<<(N-1))-1) {  // 是否比赛过
            from(j, 0, i) {  // 是否胜利
                ...
                for (int p=0; p<=N-2; p++) {
                    ...
                    if (~(i & mask)) {  // 没有比赛过
                        int exp = init[1] + e[i];
                        int exp_guest = init[p+2];

                        if (exp > exp_guest) {
                            chkMax(F[i|mask][j|mask], F[i][j] + score[1][p+2]);
                        } else {
                            chkMax(F[i|mask][j&~mask], F[i][j]);
                        }
                    }
                }
            }
        ...
        debug batchOutput2d(F, 1<<N, 1<<N, "%3d ");
