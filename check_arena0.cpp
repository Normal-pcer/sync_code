...
void insert(int& rt,int l,int r,int k,int x){
	if(!rt)rt=++tot;
	if(l==r){
		tr[rt].sum+=x;
		return;
	}
	if(k<=mid)insert(lc,l,mid,k,x);
	else insert(rc,mid+1,r,k,x);
	pushup(rt);
}
int query(int rt,int l,int r,int k){
	...
	else return query(rc,mid+1,r,k)+tr[lc].sum;
}
...
signed main(){
	...
	io>>n>>m;
	for(int i=1;i<=n;i++){
		io>>p[i].x>>p[i].y>>p[i].num;
	}
	for(int i=1;i<=m;i++){
		int pos=i*4+n-3;
		io>>p[pos].x>>p[pos].y>>p[pos+1].x>>p[pos+1].y;
//		scanf("%lld%lld%lld%lld",&p[pos].x,&p[pos].y,&p[pos+1].x,&p[pos+1].y);
		p[pos].idx=p[pos+1].idx=p[pos+2].idx=p[pos+3].idx=i;
		p[pos].sta=p[pos+1].sta=1;
		p[pos+2].sta=p[pos+3].sta=-1;
		p[pos+2].x=p[pos].x-1;
		p[pos+2].y=p[pos+1].y;
		p[pos+3].x=p[pos+1].x;
		p[pos+3].y=p[pos].y-1;
	...
	sort(p+1,p+n+4*m+1,cmp);
	for(int i=1;i<=n+4*m;i++){
		if(p[i].sta){
			q[p[i].idx]+=p[i].sta*query(1,-M-1,M,p[i].y);
		}else{
			insert(rt,-1-M,M,p[i].y,p[i].num);
		}
	//		cout<<p[i].x<<" "<<p[i].y<<" "<<p[i].idx<<" "<<p[i].sta<<endl;
	}
	for(int i=1;i<=m;i++){
		io<<q[i]<<"\n";
	}
