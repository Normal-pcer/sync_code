/**
 * @link https://www.luogu.com.cn/problem/P1171
 */
...
auto lg2 = static_cast<int(*)(int)>(std::__lg);
...
namespace Solution {
    ...
    int dis[_N][_N];
    int F[_N][1<<_N];  // 以 i 结尾，包括 j 的所有为 1 的二进制的所有位的路径的最短长度
    ...
    void solve() {
        ...
        from(j, 0, (1<<N)-1) {
            from(i, 0, N-1) {  // i 是最后到达的点
                if ((1<<i) & j)
                    bit::traverse1(j, [i, j](int p){  // 到 p 结束
                        never log("(%d)F[%d][%d], (%d)F[%d][(%d)%d^%d] + (%d)dis[%d][%d]\n", F[i][j], i, j, F[lg2(p)][j ^ p], lg2(p), j^(1<<i), j, (1<<i), dis[lg2(p)][i], lg2(p), i)
                        chkMin(F[i][j], F[lg2(p)][j ^ (1<<i)] + dis[lg2(p)][i]);
                    });
            }
        }

        int ans = Infinity;
        from(i, 0, N-1)  chkMin(ans, F[i][(1<<N)-1] + dis[i][0]);
