...
function logic(x: number, y: number): number {
    return (x & y) ^ ((x ^ y) | (~x & y));
}

function problemsIndex() {
    if (!(window.location.href.includes("list") || window.location.href.includes("training")))  return;
...
function jumpTo(element: HTMLElement) {
    console.log(element)
