...
#define force
namespace Solution_3403780003578334 {

    int N;  const int _N = 2005;

    const int mod = 998244353;

    bool check(const std::vector<int>& vec) {
        static int idx[_N];
        from(i, 1ULL, vec.size()-1)  idx[vec[i]] = i;

        // 对于所有的合法二元组
        from(i, 1ULL, vec.size()-1) {
            from(j, i+1, vec.size()-1) { 
                if (vec[i] > vec[j]) {  // 一个合法二元组 (pi, pj)
                    // 需要保证这个元组能被反向凑出，即 (j, i) 可以作为 另一种的 (pi, pj)
    ...
    void solve() {
        ...
        std::vector<int> mask;
        mask.push_back(0);
        from(i, 1, N)  mask.push_back(io.get());

        if (std::all_of(mask.begin()+1, mask.end(), lam(x, x))) {
            io << (check(mask)? 1: 0) << endl;
            return;
        }

        std::vector<int> vec;
        vec.push_back(0);
        from(i, 1, N)  vec.push_back(i);
        auto cnt = 0LL;
        do {
            from(i, 0ULL, mask.size()-1) {
                if (mask[i] != 0 and mask[i] != vec[i])  goto egg;
            }
        ...
        egg:
            ;
        } while (std::next_permutation(vec.begin()+1, vec.end()));
...
int main() {
    initDebug;
    Solution_3403780003578334::solve();
    return 0;
}