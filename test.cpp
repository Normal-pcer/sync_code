...
namespace Solution {
    ...
    public:
        ...
        void damaged(const Damage& obj) {
            ...
            // 
            if (obj.type == Killing) {
                Card *dodge = findCard(Label::D_Dodge);  // 
                if (dodge)  dodge->abandon();
                else  strength -= obj.amount;  // 
            } else if (obj.type == Invading) {  // 
                auto killing = findCard(Label::K_Killing);  // 
                if (killing)  killing->abandon();
                else  strength -= obj.amount;  // 
            } else if (obj.type == Shooting) {  // 
                auto dodging = findCard(Label::D_Dodge);  // 
            ...
            if (character == M_Master) {
                ...
                // 
                if (obj.source.impression == _Undefined and obj.type > DuelingFailed)  //* 
                    obj.source.impression = _Questionable;
            }

            bool hostility = (obj.type >= Dueling);  // 
     ...
     * @param original 
     * @param pos 
     * @param tag tag  before.owner  original 
     * @param depth 
     * 
     * @returns 
     */
    bool tryExecutingUnbreakable(const Card& before, const Player& original, 
        ...
        if (original.impression < F_Thief and original.character != M_Master) { // 
            log("Break! -> 0\n");
            return false; 
        }
        // 
    ...
    bool D_Dodge(const Card card) {
        return (card.label != Label::D_Dodge);  // 
    }
...
#define cur players[target]
        ...
        cur->damaged({*card.owner, 0, Dueling});  // 
