...
#define PARSER_DEBUG_MESSAGE do { debug { io << __PRETTY_FUNCTION__ << endl; for (auto x: src | views::take(8)) { io << x << endl; } io << "-----" << endl; io.flush(); std::fflush(stdout); } } while (false)
namespace AST {
...
std::vector<ExpressionNode *> arraySize; 
...
auto l_son = evaluateExpression(node->left);
auto r_son = evaluateExpression(node->right);
#define JOIN_INT_OP(op_name, symbol) else if (node->op == AST::ExpressionNode::op_name and l_son.type == Object::Int and r_son.type == Object::Int) { Object res(Object::Int); assert(l_son.type == l_son.Int and r_son.type == r_son.Int); res.value = static_cast<int>(std::get<int>(l_son.value) symbol std::get<int>(r_son.value)); return res; }
if (0) {}
JOIN_INT_OP(Add, +)
JOIN_INT_OP(Sub, -)
JOIN_INT_OP(Mul, *)
...
auto tokens = FutureProgram::Tokenizer{}.tokenize(scan);
auto ast = FutureProgram::AST::BlockNode::parse(tokens).node;
auto interpreter = FutureProgram::Interpreter::Interpreter(FutureProgram::Program{ast, FutureProgram::IdentifierMap{}});
interpreter.run();
} else {
FutureProgram::solve();
}
