...
template <typename T> inline auto chkMax(T& base, const T& cmp) { R (base = max(base, cmp)); }
template <typename T> inline auto chkMin(T& base, const T& cmp) { R (base = min(base, cmp)); }
#define never if constexpr(0)
const int inf = 0x3f3f3f3f; const long long infLL = 0x3f3f3f3f3f3f3f3fLL; using ll = long long; using ull = unsigned long long;
#define __lambda_1(expr) [&](){R expr;}
#define __lambda_2(a, expr) [&](auto a){R expr;}
#define __lambda_3(a, b, expr) [&](auto a, auto b){R expr;}
#define __lambda_4(a, b, c, expr) [&](auto a, auto b, auto c){R expr;}
...
R __builtin_add_overflow(x, y, &x);
}
bool subOverflow(auto &x, auto y) {
R __builtin_sub_overflow(x, y, &x);
}
bool mulOverflow(auto &x, auto y) {
R __builtin_mul_overflow(x, y, &x);
}
...
int ungetFlag = 0;
virtual char gc() = 0;
static bool isDigit(char ch) { R '0' <= ch and ch <= '9'; }
static bool isBlank(char ch) { R ch <= 32 or ch == 127; }
public:
char get() {
if (ungetFlag) {
R prev[--ungetFlag];
}
R (prev[1] = prev[0], prev[0] = gc());
}
Scanner &unget() {
...
if (sign) {
for (; isDigit(ch); ch = get()) {if (mulOverflow(x, 10)) throw IntegerOverflowError{};
...
};unordered_map<string, IdentifierIndexType> mappingStringToIndex;unordered_map<int, string> mappingIndexToString;
public:
IdentifierMap();
IdentifierIndexType getIndex(string const &s, IdentifierIndexType default_val) {
auto it = mappingStringToIndex.find(s);
if (it != mappingStringToIndex.end()) R it->second;
else R default_val;
}
...
if (it != mappingIndexToString.end()) R it->second;
...
R mappingStringToIndex.find(s) != mappingStringToIndex.end();
}
bool containsIndex(IdentifierIndexType index) {
R mappingIndexToString.find(index) != mappingIndexToString.end();
...
R ('A' <= ch and ch <= 'Z') or ('a' <= ch and ch <= 'z') or ch == '_';
}
...
string str_name; char ch = io.get();
...
R identifierMap.getString(name) == other;
}
...
value = 0;char ch = io.get();
try {
if (ch == '0') {
ch = io.get();
if (ch == 'x' or ch == 'X') {for (ch = io.get(); isDigit(ch) or ('A' <= ch and ch <= 'F') or ('a' <= ch and ch <= 'f') or ch == '\''; ch = io.get()) {
if (ch == '\'') continue;
...
io.unget();
} else if (ch == 'b' or ch == 'B') {for (ch = io.get(); ch == '0' or ch == '1' or ch == '\''; ch = io.get()) {
if (ch == '\'') continue;
if (IO::leftShiftOverflow(value, 1)) throw IO::IntegerOverflowError{};
if (IO::addOverflow(value, ch ^ 48)) throw IO::IntegerOverflowError{};
}
io.unget();
} else if (ch == 'o' or ch == 'O') {for (ch = io.get(); ('0' <= ch and ch <= '7') or ch == '\''; ch = io.get()) {
if (ch == '\'') continue;
...
egg:for (; isDigit(ch) or ch == '\''; ch = io.get()) {
if (ch == '\'') continue;
...
string value;
String(): value() {}
String(string value): value(move(value)) {}
String(string &&value): value(move(value)) {}
void parse(IO::Scanner &io) {
value.clear();
char ch = io.get();
...
[[likely]] default: 
if (isBlank(x)) io << format("\\x{:02X}", x);
else io << x;
}
}
io << '"';
...
variant<int, Identifier, Integer, String, Symbol> value = 0;
friend IO::Printer &operator<<(IO::Printer &io, const Token &token) {
switch (token.tag) {
case Token::IdentifierTag:
R io << "Identifier_" << get<Identifier>(token.value);
case Token::IntegerTag:
R io << "Integer" << get<Integer>(token.value);
case Token::StringTag:
R io << "String" << get<String>(token.value);
case Token::SymbolTag:
R io << "Symbol" << get<Symbol>(token.value);
...
struct ParseResult {
T *node;
TokenIterator it;template <typename U>
operator ParseResult<U> () const {
R {static_cast<U *>(node), it};
}
};
...
switch (op) {
case Call: R {2, false};
case Subscript: R {2, false};
case SplitComma: R {17, false};
case UnaryAdd: R {3, true};
case UnarySub: R {3, true};
case Add: R {6, false};
case Sub: R {6, false};
case Mul: R {5, false};
case Div: R {5, false};
case Mod: R {5, false};
case Less: R {9, false};
case LessEqual: R {9, false};
case Greater: R {9, false};
case GreaterEqual: R {9, false};
case Equal: R {10, false};
case NotEqual: R {10, false};
case And: R {14, false};
case Or: R {15, false};
case Not: R {3, true};
case BitAnd: R {11, false};
case BitOr: R {13, false};
case BitXor: R {12, false};
case BitNot: R {3, true};
case BitShiftLeft: R {17, false};
case BitShiftRight: R {17, false};
case Assign: R {16, true};
case MemberAccess: R {2, false};
case Range: R {14, false};
default: R {OperatorInfo::priority_max, false};
}
}
...
type = [&]() {
switch (token.tag) {
case Token::IntegerTag: R Integer;
case Token::FloatingPointTag: R FloatingPoint;
case Token::StringTag: R String;
case Token::IdentifierTag: R Identifier;
default: R unreachable(), NoneValue;
}
}();
}
};
...
enum DeclareType {
Variable, Function 
} declareType;Identifier typeName; 
...
struct RunBlockStatementNode: public StatementNode {BlockNode *block;
RunBlockStatementNode(BlockNode *block): StatementNode(StatementNode::RunBlockStatement), block(block) {}
~RunBlockStatementNode() {
delete block;
}
};
struct ConditionalStatementNode: public StatementNode {ExpressionNode *condition; 
BlockNode *body; 
...
R ConditionalStatementNode::parse<decltype(src), IfStatementNode>(src);
}
};
...
R ConditionalStatementNode::parse<decltype(src), WhileStatementNode>(src);
}
};
...
auto it = src.begin();assert(it->tag == Token::IdentifierTag);
auto type_name = get<Identifier>(it->value);
it++;assert(it->tag == Token::IdentifierTag);
auto next_identifier = get<Identifier>(it->value);
it++;auto try_read_array_size = [&]() -> vector<ExpressionNode *> {
vector<ExpressionNode *> res;
while (it->tag == Token::SymbolTag and get<Symbol>(it->value).value == "[") {auto end_condition = [](auto const &op, auto const &stack) -> bool {
if (op == "]") {
bool flag = true;
...
auto first_array_size = try_read_array_size();ExpressionNode *first_initializer = nullptr;
...
if (it->tag == Token::SymbolTag and get<Symbol>(it->value).value == ",") {
it++;auto var_declare = new VariableDeclareStatementNode{type_name};
delete res, res = var_declare;
var_declare->declares.push_back(new VariableDeclareStatementNode::SingleDeclare{next_identifier, first_initializer, first_array_size});while (true) {
...
} else if (it->tag == Token::SymbolTag and get<Symbol>(it->value).value == "(") {assert(first_initializer == nullptr);
...
} else {auto var_declare = new VariableDeclareStatementNode{type_name};
delete res, res = var_declare;
...
auto it = src.begin();if (it->tag == Token::SymbolTag and get<Symbol>(it->value).value == ";") R {res, ++it};
if (it->tag == Token::EndOfLineTag) R {res, it};
...
auto it = src.begin();
for (; it != src.end();) {if (it->tag == Token::SymbolTag and get<Symbol>(it->value).value == "}") R {res, ++it};auto [statement, next] = StatementNode::parse(TokensSubrange{it, src.end()});
...
R {new RunBlockStatementNode{sub_block}, next};
}if (it->tag == Token::IdentifierTag and get<Identifier>(it->value) == Keywords::Using) {
it += 3;
assert(it->tag == Token::SymbolTag and get<Symbol>(it->value).value == ";");
it++;
R {nullptr, it};
}if (it->tag == Token::IdentifierTag and get<Identifier>(it->value) == Keywords::If) {auto [if_statement, next] = IfStatementNode::parse(TokensSubrange{++it, src.end()});
it = next;if (it != src.end()) {while (it != src.end() and it->tag == Token::EndOfLineTag) it++;
...
if (it->tag == Token::SymbolTag and get<Symbol>(it->value).value == "{") {auto [else_block, next] = BlockNode::parse(TokensSubrange{++it, src.end()});
if_statement->elseBody = else_block, it = next;
} else {auto [else_statement, next] = StatementNode::parse(TokensSubrange{it, src.end()});
...
R WhileStatementNode::parse(TokensSubrange{++it, src.end()});
}if (it->tag == Token::IdentifierTag and get<Identifier>(it->value) == Keywords::For) {
R ForStatementNode::parse(TokensSubrange{++it, src.end()});
}if (it->tag == Token::IdentifierTag and get<Identifier>(it->value) == Keywords::Var) {R VariableDeclareStatementNode::parse(TokensSubrange{++it, src.end()});
}if (it->tag == Token::IdentifierTag and get<Identifier>(it->value) == Keywords::Def) {
R FunctionDeclareStatementNode::parse(TokensSubrange{++it, src.end()});
}if (it->tag == Token::IdentifierTag and get<Identifier>(it->value) == Keywords::Return) {
R ReturnStatementNode::parse(TokensSubrange{++it, src.end()});
}if (it->tag == Token::IdentifierTag and next(it) != src.end() and next(it)->tag == Token::IdentifierTag) {
R VariableDeclareStatementNode::parse(TokensSubrange{it, src.end()});
}auto [expr, next] = ExpressionNode::parse(TokensSubrange{it, src.end()});
R {new ExpressionEvaluateStatementNode{expr}, next};
}
template <typename T, typename Out>
...
auto it = src.begin();assert(it != src.end() and it->tag == Token::SymbolTag and get<Symbol>(it->value).value == "(");
it++;{
...
R [&](string const &op, vector<StackValueType> const &ops) {
if (op == "," and (ops.empty() or ops.back().op == NoneOp)) R true;
if (op == ";" or op == "\n") R true;
R false;
};
} else {
...
auto it = src.begin();struct PostfixValueType {
bool symbol = false;
...
ops.pop_back();if (flag) postfix.push_back({false, ValueNode{ValueNode::NoneValue, Token{Token::NoneTag}}});
} else {assert(ops.back().op == Bracket and ops.back().args_remains == 0);
ops.pop_back();
}
} else if (op == ",") {
...
} else if (op == "]") {while (infoOf(ops.back().op).priority != inf) {
postfix.push_back({true, ops.back().op}), ops.pop_back();
}
assert(ops.back().op == SubscriptBracket);
bool flag = ops.back().args_remains == 1;ops.pop_back();
...
} else {if (token.tag == Token::IdentifierTag) {
...
variant<
nullptr_t, 
shared_ptr<map<Identifier, Object>>, 
int, 
shared_ptr<string>, 
shared_ptr<Identifier>,
shared_ptr<ArrayObjectValue>,
...
Scope(Scope *parent = nullptr): parent(parent) {}Object &get(Identifier name) {
auto it = variables.find(name);
if (it != variables.end()) R it->second;
else {
if (parent != nullptr) R parent->get(name);
...
if (parent != nullptr) R parent->getPtr(name);
else R nullptr;
}
...
variables.insert({name, Object{Object::Struct, make_shared<map<Identifier, Object>>()}});
} else {
unreachable();
}
}
...
Interpreter::Interpreter(Program &&program): program(move(program)) {scopeStack.push_back(make_unique<Scope>());topScope()->variables.insert({{"putchar"}, Object{Object::BuiltinFunction, make_shared<Identifier>("putchar")}});
...
if (node->op == AST::ExpressionNode::SplitComma) {out = getFunctionArguments(node->left, out);
out = getFunctionArguments(node->right, out);
} else {
if (node->op == AST::ExpressionNode::NoneOp) {auto value_node = dynamic_cast<AST::ValueNode *>(node);
if (value_node->token.tag == Token::NoneTag) R out;
}*out++ = evaluateExpression(node);
}
R out;
}
void Interpreter::run() {
enterScope();topScope()->variables.insert({{"cin"}, Object{Object::BuiltinIStream}});
...
runStatement(x);
if (returnFlag) R leaveScope(), void();
...
if (node == nullptr) R nullptr;
if (node->op == node->NoneOp) {auto value_node = dynamic_cast<AST::ValueNode *>(node);
if (value_node->token.tag == Token::IdentifierTag) {
auto identifier = get<Identifier>(value_node->token.value);
R &topScope()->get(identifier);
} else {
R nullptr;
}
...
auto name = get<Identifier>(value_node_right->token.value);
assert(base.type == Object::Struct);
auto members = get<shared_ptr<map<Identifier, Object>>>(base.value);
R &(*members)[name];
...
R &array[get<int>(r_son.value)];
} else {
R nullptr;
}
}
Object Interpreter::evaluateExpression(AST::ExpressionNode *node) {
if (node == nullptr) R Object{Object::None};
if (node->op == node->NoneOp) {auto value_node = dynamic_cast<AST::ValueNode *>(node);
assert(value_node);
...
R Object{Object::String, shared_ptr<string>{new string(get<Tokenizer::String>(value_node->token.value).value)}};
...
R *evaluateLeftValueExpression(node);
...
unreachable();
} else {auto l_son = evaluateExpression(node->left);
auto r_son = evaluateExpression(node->right);
...
res.value = static_cast<int>(get<int>(l_son.value) symbol get<int>(r_son.value)); \
R res; \
}
if (0) {}
JOIN_INT_OP(Add, +)
...
assert(l_son.type == Object::Int);
R Object{Object::Int, -get<int>(l_son.value)};
...
runBlock(func->body);
leaveScope();
if (returnFlag) R returnFlag = false, ret;
else R Object{Object::Int, 0}; 
}
...
auto function_decl_statement = dynamic_cast<AST::FunctionDeclareStatementNode *>(node);topScope()->declare(function_decl_statement->name, TypeName{TypeName::Function});
...
real_type = {
TypeName::Array,
make_shared<ArrayMeta>(real_type, 0, get<int>(array_size.value))
};
arr.pop_back();
}
...
} else if (x->type == AST::StatementNode::ReturnStatement) {auto return_statement = dynamic_cast<AST::ReturnStatementNode *>(x);
