/**
 * @link https://neooj.com:8082/oldoj/problem.php?id=3398
 */
#include "./lib_v5.hpp"
...
namespace Solution_2868545612296582 {
    class SegTree {
        struct Node {
            i32 begin = 0, end = 0;
            i32 cnt = 0, assign_tag = -1;  // -1 表示没有赋值操作

            auto len() const -> i32 { return end - begin; }
            auto assign(bool x) -> void {
                if (x) cnt = len();
                else cnt = 0;
                assign_tag = x;
            }
        ...
        auto pushUp(i32 p) -> void {
            tree[p].cnt = tree[lson(p)].cnt + tree[rson(p)].cnt;
        }
        auto pushDown(i32 p) -> void {
            if (tree[p].assign_tag != -1) {
                tree[lson(p)].assign(tree[p].assign_tag);
                tree[rson(p)].assign(tree[p].assign_tag);
                tree[p].assign_tag = -1;
            }
        }
        auto build(i32 begin, i32 end, i32 p, std::vector<char> const &init) -> void {
    ...
    public:
        ...
        auto countRange(i32 begin, i32 end, i32 p = 1) -> i32 {
            ...
            pushDown(p);
            i32 ans = 0;
            if (tree[lson(p)].end > begin)  ans += countRange(begin, end, lson(p));
            if (tree[rson(p)].begin < end)  ans += countRange(begin, end, rson(p));
            return ans;
        }
        auto assignRange(i32 begin, i32 end, bool val, i32 p = 1) -> void {
            if (begin == end) return;
            if (tree[p].begin >= begin and tree[p].end <= end) {
                tree[p].assign(val);
                return;
    ...
    auto solve() -> void {
        ...
        std::vector<i32> a(N);
        for (auto &x: a) io >> x;
        std::vector<std::tuple<i32, i32, i32>> ops(M);
        for (auto &[op, x, y]: ops) io >> op >> x >> y, x--;
        i32 Q; io >> Q, Q--;

        auto check = [&](i32 val) -> bool {
            std::vector<char> b(a.size());

            std::transform(a.begin(), a.end(), b.begin(), lam(x, x >= val));
            SegTree sgt{N, b};

            for (auto [op, x, y]: ops) {
                auto cnt1 = sgt.countRange(x, y);
                auto cnt0 = y - x - cnt1;
                if (op == 0) {
                    sgt.assignRange(x, x + cnt0, false);
                    sgt.assignRange(x + cnt0, y, true);
                } else {
                    sgt.assignRange(x, x + cnt1, true);
                    sgt.assignRange(x + cnt1, y, false);
                }

            }

            auto ans = sgt.countRange(Q, Q + 1);
            return ans;
        };

        auto ans = [&](i32 begin, i32 end) -> i32 {
            while (end - begin >= 1) {
                auto mid = begin + ((end - begin) >> 1);
                if (check(mid)) {
                    begin = mid + 1;
...
auto main(int argc, char const *argv[]) -> int {
    ...
    Solution_2868545612296582::solve();
    return 0;
}
