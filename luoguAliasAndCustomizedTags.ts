...
class UserIdentifier {
    ...
    static fromUid(uid: number): Promise<UserIdentifier> {
        console.log(`UserIdentifier::fromUid(${uid})`);
        let res = uidToIdentifier.get(uid);
        if (res !== undefined) {
            return new Promise<UserIdentifier>((resolve, _) => {
                resolve(res);
            });
        }
        // 否则，直接通过 API 爬取
        ...
        return new Promise<UserIdentifier>((resolve, reject) => {
            xml.addEventListener("loadend", () => {
                ...
                if (xml.status === 200) {
                    ...
                    let users: Array<any> = json["users"];
                    if (users.length !== 1) {
                        reject();
                    } else {
                        let uid = users[0]["uid"];
    ...
    static fromUsername(username: string): Promise<UserIdentifier> {
     ...
     */
    static fromAny(s: string): Promise<UserIdentifier> {
        // 保证：UID 一定为数字
                ...
                if (p.classList.contains("title")) {
                    ...
                    if (a_list.length === 1) {
                        ...
                        if (span.innerText.includes(alias.id.username)) {
                            if (span.getElementsByClassName(Prefix + "alias").length !== 0) return;
                            // 尝试在里面添加一个 span 标注别名
...
let aliases: Map<UserIdentifier, UsernameAlias> = new Map();
let cache: Map<number, Map<string, unknown>> = new Map(); // 每个 UID 的缓存
...
class SettingBox {
    ...
    placed: boolean = false; // 已经被放置
    callback: ((items: Array<SettingBoxItem>) => void) | null = null; // 确定之后调用的函数
     ...
     * @param func 用作处理的函数
     */
    handle(func: ((items: Array<SettingBoxItem>) => void) | null = null): void {
        this.callback = func;
    }
    /**
     * 尝试在当前文档中放置设置块。
...
class UserTag {
    ...
    constructor(id: UserIdentifier, tag: string) {
        this.id = id;
        this.tag = tag;
    }
    ...
    apply(): void {
        ...
        for (let link of links) {
            ...
            // 已经放置过标签
            if (
                link.parentElement?.getElementsByClassName(Prefix + "customized-tag").length !== 0
            ) {
            ...
            // - 如果存在颜色属性，直接使用
            // - 否则，尝试通过 class 推断颜色
            let existsColorStyle: boolean = false;
            ...
            if (color !== "") {
                ...
                if (color.startsWith("#")) {
                    ...
                    if (rgb !== null) {
                        // 十进制转为十六进制
                        const f = (x: string) => parseInt(x).toString(16);
            ...
            // 在缓存中保存颜色信息
            if (!cache.has(this.id.uid)) cache.set(this.id.uid, new Map());
...
let tags: Map<UserIdentifier, UserTag> = new Map();
            ...
            for (let item of _cache) {
                if (item instanceof Array && item.length === 2) {
                    ...
                    if (typeof uid === "number" && typeof data === "object") {
                        let data_map = new Map<string, any>();
...
(function () {
    ...
    alias_box.handle((arr: Array<SettingBoxItem>) => {
        let uid_or_name: string = arr[0].getValue();
        let alias: string = arr[1].getValue();
    ...
    tag_box.handle((arr: Array<SettingBoxItem>) => {
        let uid_or_name: string = arr[0].getValue();
        let tag: string = arr[1].getValue();
        UserIdentifier.fromAny(uid_or_name).then((identifier) => {
    ...
    restore_box.handle((arr: Array<SettingBoxItem>) => {
        let uid_or_name: string = arr[0].getValue();
        UserIdentifier.fromAny(uid_or_name).then((identifier) => {
            let deleted_item: string[] = [];
            ...
            if (deleted_item.length > 0) {
                alert(
                    `已删除 ${identifier.username} (${identifier.uid}) 的 ${deleted_item.join(
                        "和"
                    )}（刷新网页生效）`
                );
            }
    ...
    window.onload = () => {
        ...
        function create_boxes_parent() {
            ...
            if (boxes_grand_parent.length !== 1) throw "cannot place boxes-parent";
