...
namespace Formatter {
    ...
    InfoTable toTable(const string &json) {
        ...
        auto readChar = [&](char c) {
            while (idx < json.size() && isspace(json[idx]))
                idx++;
...
string registerUser(const string &name) {
	...
	std::cout << "JSON: ";
	for (auto ch: database[uid]) {
		if (ch < 127) std::cout << ch;
		else std::cout << '$';
	}
	std::cout << endl;
...
int main() {
	try {
		...
		while (true) {
			std::string s;
			std::cin >> s;
			auto response_body = workWithRequest("register/" + s);
			response_body = workWithRequest("info/" + std::to_string(cnt++));
			std::cout << response_body << std::endl;
		}
	} catch (std::string &s) {
		std::cout << "catch: " << s << endl;
	}
    return 0;
}
