...
时间复杂度为  ![](https://www.zhihu.com/equation?tex=O(1)) 。
...
初始化列表（initializer list）是 C++11 引入的特性，可以用花括号包裹若干个**相同类型**的元素。这允许我们方便地在多个元素间取最大值或最小值。该操作的时间复杂度为  ![](https://www.zhihu.com/equation?tex=O(N))  为区间长度，下同）。

`ranges::max(range)` / `ranges::min(range)`：返回范围内的首个最大/最小元素。

时间复杂度为  ![](https://www.zhihu.com/equation?tex=O(N)) 。
...
`minmax(a, b)`：返回一个包括两个元素的 `pair`，如果 a 严格大于 b，这个返回值的顺序与传入参数相反，否则相同。时间复杂度  ![](https://www.zhihu.com/equation?tex=O(1)) 。

`minmax({...})`：返回一个 `pair`，依次包含初始化列表中的首个最小值和最后一个最大值。

`ranges::minmax(range)`：同样返回一个 `pair`，依次包含范围中的首个最小值和最后一个最大值。

后两个函数的时间复杂度均为  ![](https://www.zhihu.com/equation?tex=O(N))  次比较。

### clamp （C++17）
`clamp(min, max, x)`：将  ![](https://www.zhihu.com/equation?tex=x) ；否则返回最接近的边界。
...
`sort(begin, end)` / `ranges::sort`：对范围按照升序排序，最差情况下复杂度  ![](https://www.zhihu.com/equation?tex=O(N\cdot\log N)) 。
...
`stable_sort(begin, end)` / `ranges::stable_sort`：对范围按照升序排序，复杂度  ![](https://www.zhihu.com/equation?tex=O(N\cdot\log N)) 。这个排序是稳定的，即对于两个相等的元素，原序列中靠前的排序后一定靠前。

### partial_sort

`partial_sort(begin, mid, end)` / `ranges::partial_sort(begin, mid, end)`：重排元素，使得  ![](https://www.zhihu.com/equation?tex=[begin, mid))  区间依次包含原范围前 `distance(begin, mid)` 小的元素。

不保证稳定性，不保证  ![](https://www.zhihu.com/equation?tex=[mid, end))   中的顺序。

时间复杂度为  ![](https://www.zhihu.com/equation?tex=O(N\cdot\log M)) 。

### is_sorted

`is_sorted(begin, end)` / `ranges::is_sorted(range)`：判断范围内元素是否为单调不降，返回布尔值。时间复杂度  ![](https://www.zhihu.com/equation?tex=O(N)) 。

### is_sorted_until

`is_sorted_until(begin, end)` / `ranges::is_sorted_until(range)`：返回最后一个迭代器 `it`，使得区间  ![](https://www.zhihu.com/equation?tex=[begin, it)) 。

### nth_element

`nth_element(begin, nth, end)` / `ranges::nth_element(range, nth)`（`nth` 均为迭代器）：重排元素，使得  ![](https://www.zhihu.com/equation?tex=[begin, nth))  区间均不小于该元素，且假如将原序列排序，排序后 `nth` 指向的元素与该函数处理过的相同。

时间复杂度  ![](https://www.zhihu.com/equation?tex=O(N)) 。

## 二分查找操作

本节中的函数均支持传入比较函数，且保证序列单调不降。

本节内函数时间复杂度均为  ![](https://www.zhihu.com/equation?tex=O(\log N)) ，但要保证迭代器支持随机访问。
...
`swap(a, b)`：交换两个元素，需要二者类型相同，且可以“移动构造”和“移动赋值”。时间复杂度为  ![](https://www.zhihu.com/equation?tex=O(1)) 。

`swap(a[N], b[N])`：交换两个数组间的对应元素。值得注意的是，该操作时间复杂度为  ![](https://www.zhihu.com/equation?tex=O(N)) 。
...
时间复杂度均为  ![](https://www.zhihu.com/equation?tex=O(N)) 。

### iter_swap

`iter_swap(a, b)`：交换两个迭代器对应元素，时间复杂度  ![](https://www.zhihu.com/equation?tex=O(1)) 。

## 搜索操作
我们称呼一个返回布尔类型的、接受  ![](https://www.zhihu.com/equation?tex=N)  个参数的函数为 “N 元谓词”。以下参数的 `p` 均为谓词。
...
`xxx_of(begin, end, p)` / `ranges::xxx_of(range, p)`：区间内是否全部/包含/不含符合谓词 p 的元素，复杂度  ![](https://www.zhihu.com/equation?tex=O(N)) 。

### ranges::contains，ranges::contains_range（C++23）
`ranges::contains(range, x)`：返回布尔类型，区间内是否包含元素 x，复杂度  ![](https://www.zhihu.com/equation?tex=O(N)) 。

`ranges::contains_range(range1, range2)`：返回布尔类型，区间 range2 是否为 range1 的子区间，复杂度  ![](https://www.zhihu.com/equation?tex=O(N)) 。
...
### find_first_of
`find_first_of(begin1, end1, begin_s, end_s)` / `ranges::find_first_of(range1, range_s)`：查找第一个区间的第一个元素，使得它也位于第二个区间中，返回其迭代器。可以传入等价谓词。时间复杂度  ![](https://www.zhihu.com/equation?tex=O(N\cdot M))  分别为两个区间的长度，下同）。

### adjacent_find
`adjacent_find(begin, end)` / `ranges::adjacent_find(range)`：查找第一个元素，使得它和它后面的元素相同。时间复杂度  ![](https://www.zhihu.com/equation?tex=O(N)) ，可以传入等价谓词。
...
复杂度均为  ![](https://www.zhihu.com/equation?tex=O(N)) 。

### mismatch
`mismatch(begin1, end1, begin2)` / `mismatch(begin1, end1, begin2, end2)` / `ranges::mismatch(range1, range2)`：返回两个区间首个不同元素的迭代器组成的 `pair` 或 `mismatch_result`，可以传入等价谓词。如果区间长度不同，在到达其中一个的结尾之后停止匹配。时间复杂度  ![](https://www.zhihu.com/equation?tex=O(N)) 。

### equal
`equal(begin1, end1, begin2)` / `equal(begin1, end1, begin2, end2)` / `ranges::equal(range1, range2)`：返回布尔值，两个区间是否相同。如果长度不同返回 false。可以传入等价谓词。复杂度  ![](https://www.zhihu.com/equation?tex=O(N)) 。

### search
`search(begin1, end1, begin2, end2)` / `ranges::search(range1, range2)`：返回第二个区间在第一个区间内第一次出现的位置，返回其首迭代器/查找到的子区间。可以传入等价谓词。复杂度  ![](https://www.zhihu.com/equation?tex=O(N\cdot M)) 。
...
### search_n
`search_n(begin, end, cnt, val)` / `ranges::search_n(range, cnt, val)`：在区间中查找连续 `cnt` 个 `val`，返回第一个元素的迭代器/查找到的子区间。复杂度  ![](https://www.zhihu.com/equation?tex=O(N)) 。

### ranges::starts_with，ranges::ends_with（C++23）
`ranges::xxx_with(range1, range2)`：返回布尔值，第一个区间是否以第二个开头或结尾。

## 顺序变更操作
本节的函数复杂度均为  ![](https://www.zhihu.com/equation?tex=O(N)) 。

### reverse
`reverse(begin, end)` / `ranges::reverse(range)`：翻转区间内的元素。

### rotate
`rotate(begin, mid, end)` / `ranges::rotate(range, mid)`：将  ![](https://www.zhihu.com/equation?tex=[begin, mid))  子区间。

### shift_left，shift_right（C++20）
`shift_left(begin, end, n)`：左移范围内的元素，返回迁移后区间结尾。例如  ![](https://www.zhihu.com/equation?tex=[1, 2, 3, 4, 5]) 。
...
以下操作的复杂度大多为  ![](https://www.zhihu.com/equation?tex=O(N)) 。
...
## 移除操作
对于这些操作，常规函数一般返回新的尾迭代器，ranges 算法一般返回  ![](https://www.zhihu.com/equation?tex=[new_end, end))  的子区间。
...
## 划分操作
“划分”操作是重排序一个范围内的所有元素，使得存在一个元素，对于一个指定的谓词 p，左侧的元素都返回 true，右侧都返回 false。复杂度  ![](https://www.zhihu.com/equation?tex=O(N)) 。
...
### partition_point
`partition_point(begin, end, p)`：寻找按 p 划分好的范围的划分点，返回第一个不满足 p 的元素的迭代器。复杂度  ![](https://www.zhihu.com/equation?tex=O(\log N)) 。


## 归并操作
### merge
`merge(begin1, end1, begin2, end2, begin_out)`：合并两个已经有序的序列到目标迭代器，成为一个有序的序列，时间复杂度  ![](https://www.zhihu.com/equation?tex=O(N)) 。

可以传入比较函数。

### inplace_merge
`inplace_merge(begin, mid, end)`：保证  ![](https://www.zhihu.com/equation?tex=[begin, mid)) 。

可以传入比较函数。

这个函数的空间复杂度也是  ![](https://www.zhihu.com/equation?tex=O(N))  的。
...
### make_heap
`make_heap(begin, end)`：在指定范围上建立一个大根堆。时间复杂度  ![](https://www.zhihu.com/equation?tex=O(N)) 。

### push_heap
`push_heap(begin, end)`：将迭代器 `end - 1` 指向的元素插入到  ![](https://www.zhihu.com/equation?tex=[begin, end-1)) 。

### pop_heap
`pop_heap(begin, end)`：移除这个堆中最大的元素，即将 `begin` 和 `end - 1` 指向的元素交换，接着维护  ![](https://www.zhihu.com/equation?tex=[begin, end-1))  上的大根堆。

### is_heap
`is_heap(begin, end)`：返回布尔值，判断范围内是否为一个合法的堆。时间复杂度为  ![](https://www.zhihu.com/equation?tex=O(N)) 。

### is_heap_until
`is_heap_until(begin, end)` / `ranges::is_heap_until(begin, end)`：判断从 begin 开始能形成的最大堆，返回其尾迭代器/子区间。时间复杂度为  ![](https://www.zhihu.com/equation?tex=O(N)) 

## 字典序
下列函数可以按字典序比较两个范围内的元素，时间复杂度均为  ![](https://www.zhihu.com/equation?tex=O(N)) 。
...
下列函数的时间复杂度均为  ![](https://www.zhihu.com/equation?tex=O(N)) 。

### iota
`iota(begin, end, value)` / `ranges::iota(range, value)`：将区间赋成从 value 开始的递增序列。

### accumulate
`accumulate(begin, end, init, op)`：将序列中的元素依次“累加”到 init 上，类似  ![](https://www.zhihu.com/equation?tex=op(op(init, a_1), a_2)\dots)  的方式运算。不传入 `op` 默认为求和。

不要求 init 和元素类型相同。

### inner_product
`inner_product(begin1, end1, begin2, init)`：将两个序列看成  ![](https://www.zhihu.com/equation?tex=N)  维向量，计算二者的数量积并加上 init。即 

![](https://www.zhihu.com/equation?tex=init+\sum\limits_{1\le i\le N}a_ib_i)
的值。

`inner_product(begin1, end1, begin2, init, op1, op2)`：用 `op1` 替换上文的加法，`op2` 替换乘法。即对于每一次迭代：

![](https://www.zhihu.com/equation?tex=init\leftarrow op1\left(init, op2\left(x, y\right)\right) )
...
### partial_sum
`partial_sum(begin, end, begin2, op)`：计算前缀和，输出的第  ![](https://www.zhihu.com/equation?tex=i)  项通过 `op` 累加的结果。不传入 `op` 默认为加法。
...
### exclusive_scan
行为类似于 `inclusive_scan`，但是输出的第  ![](https://www.zhihu.com/equation?tex=i)  项进行累加。
...
### fold_left
`fold_left(range, init, op)`：在 `init` 的基础上，通过 `op` 求和。即表达式  ![](https://www.zhihu.com/equation?tex=op(op(init, a_1), a_2)\dots) 

### fold_left_first
`fold_left_first(range, op)`：在范围首项的基础上，通过 `op` 求和。返回值为 `optional`。

### fold_right
`fold_right(range, init, op)`：在 `init` 的基础上，通过 `op` 倒序求和。即表达式  ![](https://www.zhihu.com/equation?tex=\dots op(a_{N-1}, op(a_N, init))) 。
