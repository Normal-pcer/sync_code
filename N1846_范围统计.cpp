/**
 * @link https://neooj.com:8082/oldoj/problem.php?id=1849
 */
...
namespace Solution {
    ...
    void solve() {
        init();
        upto(_, M) {
            int a = io.read<int>(), b = io.read<int>();

            int a_idx = std::lower_bound(x, x+N, a) - x;
            int b_idx = std::upper_bound(x, x+N, b) - x;
            printf("%d\n", b_idx - a_idx);
        }
    }
}
