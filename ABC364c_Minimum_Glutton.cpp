...
namespace Solution {
    ...
    void init() {
        io >> N >> X >> Y;
        upto(i, N)  io >> ds[i].sweet;
        upto(j, N)  io >> ds[j].salty;
    }
    ...
    void solve() {
        ...
        // Sweet
        std::sort(ds+1, ds+N+1, [](dish x, dish y){return x.sweet > y.sweet;});
        ll ans1 = 1, sum = ds[1].sweet;
        while (sum <= X && ans1<=N) {
            ans1++;
            sum += ds[ans1].salty;
        }
        ...
        // Salty
        std::sort(ds+1, ds+N+1, [](dish x, dish y){return x.salty > y.salty;});
        ll ans2 = 1, sum2 = ds[1].salty;
        while (sum2 <= Y && ans2<=N) {
            ans2++;
            sum2 += ds[ans2].salty;
        }
