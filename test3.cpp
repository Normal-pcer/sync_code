...
static ParseResult parse(const std::vector<Token> &);
...
if (op == NoneOp) {
delete dynamic_cast<ValueNode *>(this);  // 删除值节点管理的资源
} else {
if (left)  delete left;
if (right)  delete right;
}
}
ExpressionNode::ParseResult ExpressionNode::parse(const std::vector<Token> &src) {
auto [postfix, it] = [&]() {                
...
assert(v_root.token.tag == Compiler::Token::IntegerTag);
...
auto *root = Compiler::AST::ExpressionNode::parse(res).node;
io << calc(root) << endl;
}
void solve() {
test();
