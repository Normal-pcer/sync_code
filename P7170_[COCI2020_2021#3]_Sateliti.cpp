...
namespace Solution_6710300631688193 {
    ...
    void solve() {
        ...
        for (auto i: range(N)) {
            ...
            matrix[i + N] = matrix[i];
            matrix[i].shrink_to_fit(), matrix[i + N].shrink_to_fit();
        }
        N += N, M += M;
        auto lgN = std::__lg(N) + 1, lgM = std::__lg(M) + 1;
        auto constexpr const MaxLogN = 12; 

        std::vector F(N, std::vector<std::array<unsigned int, MaxLogN>>(M));
        std::vector G(M, std::vector<std::array<unsigned int, MaxLogN>>(N));
        for (auto i: range(N)) {
            for (auto j: range(M)) {
                F[i][j][0] = matrix[i][j];
            }
