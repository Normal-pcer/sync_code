...
inline Tree meg(Tree &x,Tree &y){
	if (x.maxn==y.maxn) return {x.maxn,max(x.pos,y.pos)};
	else if (x.maxn>y.maxn) return x;
	else return y;
}
...
int main(){
	...
	for (int i=lim-1;i;i--) tr[i]=meg(tr[i<<1],tr[i<<1|1]);
	while (q--){
		int op,x,y;
		read(op,x,y);
		if (op==1){
			int l=lim,r=x+lim+1;
			while (l^r^1){
				if (l&1^1) tr[l^1].maxn+=y,tag[l^1]+=y;
				if (r&1) tr[r^1].maxn+=y,tag[r^1]+=y;
				l>>=1,r>>=1;
				tr[l]=meg(tr[l<<1],tr[l<<1|1]);
				tr[l].maxn+=tag[l];
				tr[r]=meg(tr[r<<1],tr[r<<1|1]);
				tr[r].maxn+=tag[r];
			}
			for (l>>=1;l;l>>=1){
				tr[l]=meg(tr[l<<1],tr[l<<1|1]);
			...
			Tree ansl,ansr;
			while (l^r^1){
				if (l&1^1) ansl=meg(ansl,tr[l^1]);
				if (r&1) ansr=meg(tr[r^1],ansr);
				l>>=1,r>>=1;
				ansl.maxn+=tag[l];
				ansr.maxn+=tag[r];
			}
			write(max(0,x-meg(ansl,ansr).pos),'\n');
		}
	}
	return 0;
}