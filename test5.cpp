...
constexpr inline ui Zinv(const ui x){ui y=x*(2-x*x);y*=2-x*y;y*=2-x*y;return y*(2-x*y);}
constexpr inline ui qpow_mod(ui a,ui b,const ui mod){
	ull res=1;
	for (;b;b>>=1,a=a*a%mod) if (b&1) res=res*a%mod;
	return res;
}
inline constexpr ui primitive_root(ui m){
	if (m==2) return 1;
	...
	ui divs[32]={2},cnt=1,x=(m-1)>>1;
	while ((x&1)==0) x>>=1;
	for (ui i=3;i*i<=x;i+=2){
		if (x%i==0){
			divs[cnt++]=i;
			do{
				x/=i;
...
template<ui _m=998244353>
class Poly{
public:
	inline static constexpr ui mod=_m,PP=mod<<1,PPP=mod+PP,R=-Zinv(mod),RR=-ull(mod)%mod;
...
public:
	static inline ui cipolla(ui n){
		...
		ui res=out(qpowc({a,one},(mod+1)>>1));
...
private:
	...
	static inline void init_root(const ui n){
		...
		renew(rt,nrt,n>>1,align_val_t{32}),renew(irt,nrt,n>>1,align_val_t{32});
		renew(rt4,nrt>>3,n>>3,align_val_t{32}),renew(irt4,nrt>>3,n>>3,align_val_t{32});
		renew(rt2,nrt>>3,n>>3,align_val_t{32}),renew(irt2,nrt>>3,n>>3,align_val_t{32});
		renew(rt1,nrt>>3,n>>3,align_val_t{32}),renew(irt1,nrt>>3,n>>3,align_val_t{32});
		...
		for (;m!=(n>>3);m++){
			rt4[m]=make_4(rt[m]),irt4[m]=make_4(irt[m]);
	...
	inline static ui nnum;
public:
	inline static ui *seq_num,*seq_inv,*seq_fac,*seq_ifac;
	static inline void init_seq(ui n){
		if (nnum>=n) [[likely]] return;
		n=max(n,2u);
		renew(seq_num,nnum,n,align_val_t{32}),renew(seq_inv,nnum,n,align_val_t{32});
		renew(seq_fac,nnum,n,align_val_t{32}),renew(seq_ifac,nnum,n,align_val_t{32});
...
private:
	...
	static inline void dit(ui *a,const ui n){
		...
		if (n==2){
			ui x=*a,y=a[1];
			*a=div2(add(x,y)),a[1]=div2(sub(x,y));
			return;
		}
		if (n==4){
			ui x=*a,y=a[1],u=a[2],v=a[3];
	...
	static inline void inplace_integ(ui *a,const ui l,const ui r){
		init_seq(r);
		if (r-l>7) [[likely]] for (ui i=r;i!=l;) i-=8,cast(a+i)=mul(_mm256_loadu_si256((const __m256i_u*)(a+i-1)),cast(seq_inv+i));
		else for (ui i=r;i!=l;) i--,a[i]=mul(a[i-1],seq_inv[i]);
	}
	...
	static inline void _pow_impl(const ui *a,const ui n,ui *b,const ui k_mod_p,const ui k_mod_phi_p,const ui k_min_n){
		if (n==0) [[unlikely]] return;
		if (k_min_n==0){
			*b=one;
			zero_n(b+1,n-1);
		...
		ui i=0;
		while (i!=n and a[i]==0) i++;
		if (ull(i)*k_min_n>=n) zero_n(b,n);
		else if (i){
			const ui m=n-i*k_min_n;
			alignas(32) ui c[m],d[m];
			copy_n(a+i,m,c);
			_pown0_impl(c,m,d,k_mod_p,k_mod_phi_p);
			zero_n(b,i*k_min_n);
			copy_n(d,m,b+i*k_min_n);
		}else{
	...
	static inline void _icomp_impl(const ui *a,const ui n,ui *b){
		...
		for (ui i=1;i!=n;i++) v=mul(v,w),c[i]=mul(a[i],v);
		*c=0,*d=one;
		zero_n(c+n,m-n);
		zero_n(d+1,m-1);
...
public:
	...
	inline void resize(const ui x){
		...
		ui tmp=lim;
		lim=getlim(x);
		if (tmp!=lim) renew(a,min(n,tmp),lim,align_val_t{32});
	...
	inline void swap(Poly &b) noexcept{
		std::swap(lim,b.lim),std::swap(n,b.n),std::swap(a,b.a);
	}
	Poly(Poly &&b) noexcept{
		swap(b);
	}
	~Poly(){
	...
	inline Poly& operator = (Poly &&b) noexcept{
		swap(b);
		return *this;
	}
	...
	inline Poly& mulFix(Poly b){
		const ui l=getlim(n+b.n-1),tmp=n;
		resize(l),b.resize(l);
		dif(a,l),dif(b.a,l);
		mul_n(a,b.a,l);
		dit(a,l);
...
#define make_ui_operator(op) inline Poly operator op (const ui x)const{Poly res(*this);return (res op##=x);}
	make_poly_operator(+) make_poly_operator(-) make_poly_operator(*)
	make_ui_operator(+) make_ui_operator(-) make_ui_operator(*) make_ui_operator(/)
#undef make_poly_operator
#undef make_ui_operator
...
#define make_poly_func(f) friend inline Poly f(const Poly &a){Poly b(a.n);_##f##_impl(a.a,a.n,b.a);return b;}
	make_poly_func(inv) make_poly_func(diff) make_poly_func(integ) make_poly_func(log) make_poly_func(exp) make_poly_func(sqrt)
#undef make_poly_func
	...
	friend inline Poly pow(const Poly &f,const string &str){
		const ui n=f.n;
		ull k=0,kp=0,km=0;
		for (auto c:str) k=(k*10+(c^48))%mod,kp=(kp*10+(c^48))%(mod-1),km=min<ull>(km*10+(c^48),n);
		Poly res(n);
	...
	friend Poly asin(const Poly& p){
		return integ(diff(p).mulFix(inv(sqrt(-p*p+one))));
	}
	friend Poly acos(const Poly &p){
		return integ(-diff(p).mulFix(inv(sqrt(-p*p+one))));
	}
	friend Poly atan(const Poly& p){
		return integ(diff(p).mulFix(inv(p*p+one)));
	}

	static Poly stirling1_row(const ui n){
		if (n==1) return Poly({0,one});
		else if (n&1){
			Poly f=stirling1_row(n-1);
			f.resize(n+1);
			init_seq(n+1);
			for (ui i=n;i>=1;i--) f[i]=add(f[i-1],mul(f[i],seq_num[n-1]));
	...
	static Poly stirling1_col(const ui n,const ui k){
		Poly f(n+1);
		if (k>n) return f;
		init_seq(n+1);
		f[0]=f[1]=one;
	...
	static Poly stirling2_row(const ui n){
		Poly f(n+1),g(n+1);
		init_seq(n+1);
		for (ui i=0;i<=n;i++){
			f[i]=qpow(seq_num[i],n,seq_ifac[i]);
			g[i]=(i&1?neg(seq_ifac[i]):seq_ifac[i]);
		}
		return f.mulFix(g);
	}
	static Poly stirling2_col(const ui n,const ui k){
		Poly f(n+1);
		init_seq(n+1);
	...
	friend inline Poly chirpZ(const Poly &p,const ui c,const ui m){
		const ui n=p.n;
		Poly f(n+m-1),g(n);
		for (ui i=0;i<n;i++) g[i]=qpow(c,mod-1-((ull(i-1)*i)>>1)%(mod-1),p[i]);
		for (ui i=0;i<n+m-1;i++) f[n-1+m-1-i]=qpow(c,((ull(i-1)*i)>>1)%(mod-1));
		f.mulFix(g);
		g.resize(m);
		for (ui i=0;i<m;i++) g[i]=qpow(c,mod-1-((ull(i-1)*i)>>1)%(mod-1),f[n-1+m-1-i]);
		return g;
	}
private:
	inline static vector<Poly> _eval_tr;
	static inline constexpr ui _get_tr_id(ui l,ui r){
		return (l+r)|(l!=r);
	}
...
private:
	...
	static Poly _solve(ui l,ui r,const Poly& a){
...
public:
	...
	friend ui BostanMori(Poly f,Poly g,ll n){
		for (ui i;n;n>>=1){
			Poly t(g);
			for (i=1;i<t.n;i+=2) t[i]=neg(t[i]);
			f*=t,g*=t;
			for (i=(n&1);i<f.n;i+=2) f[i>>1]=f[i];
			f.resize(i>>1);
			if (f.n==0) return 0;
			for (i=0;i<g.n;i+=2) g[i>>1]=g[i];
			g.resize(i>>1);
		}
		return qpow(g[0],mod-2,f[0]);
	}
	...
	friend Poly BerlekampMassey(const Poly& a){
		...
		for (ui i=0;i<a.n;i++){
			...
			if (a.a[i]==t) continue;
			if (k==-1u){
				k=i,d=sub(a.a[i],t),res=Poly(i+1);
				continue;
			}
			Poly u(res);
	...
	inline Poly operator ()(const Poly &g) const{return composition(*this,g);}
	...
	friend Poly pointValueTranslation(const Poly& f,const ui m){
		...
		for (ui i=1;i<=(n<<1|1);i++) mfac[i]=mul(mfac[i-1],in(m-n+i-1));
		mifac[n<<1|1]=qpow(mfac[n<<1|1],mod-2);
		for (ui i=(n<<1|1);i>0;i--){
			mifac[i-1]=mul(mifac[i],in(m-n+i-1));
...
inline constexpr u128 add(u128 a,u128 b,const u128 mod){
	return (a+=b)>=mod?a-mod:a;
}

inline constexpr ll qpow(ll a,ll b,const ui mod){
	ll res=1;
	for (;b;b>>=1,a=a*a%mod) if (b&1) res=res*a%mod;
	return res;
}

inline constexpr u128 qmul(u128 a,u128 b,const u128 mod){
	u128 res=0;
	for (;b;b>>=1,a=add(a,a,mod)) if (b&1) res=add(res,a,mod);
	return res;
}
...
struct pack_size<T,x,args...>{
	static constexpr ui value=pack_size<T,args...>::value+1;
};
template<typename T>
struct pack_size<T>{
	static constexpr ui value=0;
};
template<typename T,T ...args>
constexpr ui pack_size_v=pack_size<T,args...>::value;

template<ui i,ui N,typename A,typename B>
void _assign_recursion(const A &a,const B &b,const ui p,const ui x){
	if constexpr (i==N){
		return;
	}else{
        static_assert(
            std::is_same_v<
                decltype(get<0>(b)),
                Poly<167772161> const &
            >
        );
		ui ai=get<i>(a);
		get<i>(b).a[p]=x%ai;
		cout<<"111\n";
		_assign_recursion<i+1,N,A,B>(a,b,p,x);
	}
}

template<ui i,ui N,typename A,typename B>
u128 _crt_recursion(u128 mod_mul,const A &a,const B &b,const ui p){
	if constexpr (i==N){
		return 0;
	}else{
		ui ai=get<i>(a),bi=get<i>(b).a[p];
		u128 m=mod_mul/ai;
		ui x=qpow(ui(m%ai),ai-2,ai);
		return add(qmul(m*x,bi,mod_mul),_crt_recursion<i+1,N,A,B>(mod_mul,a,b,p),mod_mul);
	}
}

template<ui i,ui N,typename B>
void _resize_recursion(B &b,const ui x){
	if constexpr (i==N){
		return;
	}else{
		get<i>(b).resize(x);
		_resize_recursion<i+1,N,B>(b,x);
	}
}

template<ui ..._m>
class MPoly{
	static inline constexpr ui nums_count=pack_size_v<ui,_m...>;
    static inline constexpr std::tuple<decltype(_m)...> mods{_m...};
    static inline constexpr u128 mod_mul=(u128(_m)*...);
    std::tuple<Poly<_m>...> a{};
	struct Fastmod{
		int f,l;
		ull m,d;
		...
		inline friend ull operator / (const ull n,const Fastmod &m){
	...
	// 	MPoly *ptr=nullptr;
	// 	reference(){}
	// 	reference(ui p,MPoly *pt){pos=p,ptr=pt;}
	// 	inline void operator = (ui x){
	// 		_assign_recursion<0,ptr->nums_count>(ptr->mods,ptr->a,pos,x);
	// 	}
	// 	operator ui(){
	// 		return _crt_recursion<0,ptr->nums_count>(ptr->mod_mul,ptr->mods,ptr->a,pos);
	// 	}
	// };
public:
	void setmod(ui x){
	...
	MPoly(){}
	// inline reference operator [](const ui x){
	// 	return reference(x,this);
	// }
	// inline const ui operator [](const ui x) const{
	// 	return _crt_recursion<0,nums_count>(mod_mul,mods,a,x)%fm;
	// }
	void assign(ui p,ui x){
		_assign_recursion<0,nums_count>(mods,a,p,x);
	}
	ui at(ui p){
		return _crt_recursion<0,nums_count>(mod_mul,mods,a,p)%mod;
	}
	void resize(ui x){
		_resize_recursion<0,nums_count>(a,x);
	}
};
